...
export async function GET(request: Request) {
  ...
  const { data, error } = await getSupabaseServer().from("hpc_evidence").select("id,learner_profile_id,activity_id,source_type,content,attachment_reference,review_status,sufficiency_status,observed_at").eq("school_id", profile.school_id).order("observed_at", { ascending: false });
...
export async function POST(request: Request) {
  ...
  const { data: learner } = await db.from("hpc_learner_profiles").select("id,academic_year,grade").eq("id", learnerProfileId).eq("school_id", profile.school_id).maybeSingle();
  const grade = Number(learner?.grade);
  if (!learner || !Number.isInteger(grade)) return Response.json({ error: "The selected learner's HPC stage is not configured." }, { status: 400 });
  const { data: stageTemplate } = await db.from("hpc_stage_templates").select("framework_version_id,hpc_framework_versions!inner(status)").eq("is_active", true).eq("hpc_framework_versions.status", "approved").lte("grade_from", grade).gte("grade_to", grade).order("created_at", { ascending: false }).limit(1).maybeSingle();
  if (!stageTemplate?.framework_version_id) return Response.json({ error: "The selected learner's approved stage framework is unavailable." }, { status: 400 });
  ...
  const { data, error } = await db.from("hpc_evidence").insert({ school_id: profile.school_id, learner_profile_id: learner.id, activity_id: String(body.activityId || "") || null, academic_year: learner.academic_year, framework_version_id: stageTemplate.framework_version_id, source_type: sourceType, contributor_type: contributor, contributor_user_id: profile.id, content, attachment_reference: String(body.attachmentReference || "").trim() || null, review_status: "teacher_review_required", sufficiency_status: "teacher_review_required" }).select("id,source_type,content,review_status").single();
