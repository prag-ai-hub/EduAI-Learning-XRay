...
function BulkAnalysisWizard({assessment,open,done}:any){
  ...
  const startQueue=()=>{saveBulkAnalysisQueue(assessment.id,pending.map(file=>file.id));if(nextPending)open(`grade-file:${nextPending.id}`);else setStep(5)};
...
async function downloadAssessmentZip(assessment:Assessment,result:GradeResult,guide?:Worksheet,worksheet?:Worksheet){
  ...
  const gapBody=result.gaps.map((g,index)=>`<section class="topic"><h2>${index+1}. ${htmlEscape(g.concept)}</h2><p><b>What happened:</b> ${htmlEscape(g.finding||"")}</p><p><b>Foundation gap:</b> ${htmlEscape(g.foundationGap||g.prerequisiteConcept||"No earlier foundation gap was supported by the evidence.")}</p><p><b>Recommended order:</b> ${htmlEscape((g.remediationSequence||[]).join(" → ")||g.rework||"")}</p></section>`).join("");
  const reportBlob=await createBrandedPdfBlob("Learning Gap Report",`${result.studentName} · ${assessment.title}`,gapBody||"<p>No evidence-supported learning gap was identified.</p>");
  files[folder+"Learning_Gap_Report.pdf"]=new Uint8Array(await reportBlob.arrayBuffer());
  const missing:string[]=[];
  if(guide?.guide){const topics=(guide.guide.topics||[]).map((t:any)=>`<section class="topic"><h2>${htmlEscape(t.concept)}</h2><p>${htmlEscape(t.explanation)}</p><div class="answer">${htmlEscape(t.workedExample)}</div></section>`).join("");const blob=await createBrandedPdfBlob("Study Guide",`${result.studentName} · ${assessment.title}`,topics);files[folder+"Study_Guide.pdf"]=new Uint8Array(await blob.arrayBuffer())}else missing.push("Study Guide");
  if(worksheet?.content){const questions=[...(worksheet.content.mcqQuestions||[]),...(worksheet.content.subjectiveQuestions||[])].map((q:any,i:number)=>`<p><b>${i+1}.</b> ${htmlEscape(q.question)}</p>`).join("");const worksheetBlob=await createBrandedPdfBlob("Worksheet",`${result.studentName} · ${assessment.title}`,questions);files[folder+"Worksheet.pdf"]=new Uint8Array(await worksheetBlob.arrayBuffer());const answers=[...(worksheet.content.mcqQuestions||[]).map((q:any,i:number)=>`<p><b>${i+1}.</b> ${htmlEscape(q.options?.[q.correctIndex]||"")}</p>`),...(worksheet.content.subjectiveQuestions||[]).map((q:any,i:number)=>`<p><b>${i+1+(worksheet.content?.mcqQuestions?.length||0)}.</b> ${htmlEscape(q.modelAnswer||"")}</p>`)].join("");const answerBlob=await createBrandedPdfBlob("Answer Key",`${result.studentName} · ${assessment.title}`,answers);files[folder+"Answer_Key.pdf"]=new Uint8Array(await answerBlob.arrayBuffer())}else missing.push("Worksheet","Answer Key");
  if(missing.length){const statusBlob=await createBrandedPdfBlob("Resource Generation Status",`${result.studentName} · ${assessment.title}`,`<h2>Resources still to generate</h2><p>${htmlEscape(missing.join(", "))}</p><p>Generate these resources in Learning X-Ray, then download the ZIP again. Every file in this ZIP is provided in PDF format.</p>`);files[folder+"Resource_Generation_Status.pdf"]=new Uint8Array(await statusBlob.arrayBuffer())}
