...
function studyGuideDocumentBody(guide:any){const topics=(guide?.topics||[]).map((t:any,i:number)=>`<section class="topic"><span class="label">Topic ${i+1}</span><span class="label">${htmlEscape(t.mastery)}% starting mastery</span><h2>${htmlEscape(t.concept)}</h2><h3>Why this matters</h3><p>${htmlEscape(t.diagnosis)}</p><h3>Learning objective</h3><p>${htmlEscape(t.learningObjective)}</p><h3>Clear explanation</h3><p>${htmlEscape(t.explanation)}</p><h3>Worked example</h3><div class="answer">${htmlEscape(t.workedExample)}</div><h3>Guided practice</h3><ol>${(t.practiceSteps||[]).map((x:string)=>`<li>${htmlEscape(x)}</li>`).join("")}</ol><h3>Check for understanding</h3><ol>${(t.checkForUnderstanding||[]).map((x:string)=>`<li>${htmlEscape(x)}</li>`).join("")}</ol></section>`).join("");return `<p>${htmlEscape(guide?.overview||"")}</p>${topics}`}
function downloadStudyGuide(r:any,guide:any){downloadDocument(r.title||"Study-Guide",r.title||"Study Guide",`${r.subject||"Subject"} · ${r.grade||"Class not set"} · ${r.studentName||"Student"}`,studyGuideDocumentBody(guide))}
