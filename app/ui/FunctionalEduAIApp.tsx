...
import HpcTrackingStatus from "./HpcTrackingStatus";
...
function useHpcLearnerUpdates(setLearners: (learners:any[])=>void){
  useEffect(()=>{const refresh=(event:Event)=>setLearners((event as CustomEvent<any[]>).detail);window.addEventListener("hpc-learners-updated",refresh);return()=>window.removeEventListener("hpc-learners-updated",refresh)},[setLearners]);
}

function HpcLearnerProfiles(){
  const [students,setStudents]=useState<any[]>([]),[studentId,setStudentId]=useState("");
...
function HpcPromptOneWorkspace(){
  ...
  useHpcLearnerUpdates(setLearners);
...
function HpcPromptTwoForms(){
  const [rubricActivityId,setRubricActivityId]=useState("");
...
function HpcEvidenceDashboard(){
  const [learners,setLearners]=useState<any[]>([]),[learnerId,setLearnerId]=useState(""),[dashboard,setDashboard]=useState<any>(null),[message,setMessage]=useState(""),[shareUrl,setShareUrl]=useState("");
  useHpcLearnerUpdates(setLearners);
...
function HpcAnnualReporting({profile}:{profile:DemoProfile}){
  const [learners,setLearners]=useState<any[]>([]),[learnerId,setLearnerId]=useState(""),[data,setData]=useState<any>(null),[principal,setPrincipal]=useState<any>(null),[busy,setBusy]=useState(false),[message,setMessage]=useState("");
  useHpcLearnerUpdates(setLearners);
  const load=(id=learnerId)=>{if(!id)return;setBusy(true);void authFetch(`/api/hpc/annual-reports?learnerId=${encodeURIComponent(id)}`).then(async r=>{const p=await r.json();if(!r.ok)throw new Error(p.error);setData(p)}).catch(e=>setMessage(e instanceof Error?e.message:"Unable to load annual HPC workspace.")).finally(()=>setBusy(false))};
  useEffect(()=>{void authFetch("/api/hpc/learners").then(r=>r.json()).then(p=>{const list=p.learners||[];setLearners(list);setLearnerId(list[0]?.id||"")});if(profile.role==="Principal"||profile.role==="Admin")void authFetch("/api/hpc/principal-dashboard").then(r=>r.ok?r.json():null).then(setPrincipal)},[]);useEffect(()=>{if(learnerId)load(learnerId)},[learnerId]);
  const submit=async(form:HTMLFormElement,action:"draft"|"validate"|"finalize")=>{const f=new FormData(form);setBusy(true);setMessage("");try{const r=await authFetch("/api/hpc/annual-reports",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({learnerId,action,narrativeText:f.get("narrative"),teacherNotes:f.get("notes"),strengthsText:f.get("strengths"),supportText:f.get("support"),teacherApproval:f.get("approval")==="on"})}),p=await r.json();if(!r.ok){setData((current:any)=>({...current,validation:p.validation||current?.validation}));throw new Error(p.error)}setData((current:any)=>({...current,...p,summary:p.summary||current?.summary}));setMessage(action==="finalize"?"Annual HPC finalized with an immutable evidence snapshot.":action==="validate"?"Finalization checks refreshed.":"Evidence-grounded narrative draft saved for teacher review.")}catch(e){setMessage(e instanceof Error?e.message:"Unable to save annual HPC.")}finally{setBusy(false)}};
  const download=()=>{if(!data?.summary)return;const learner=data.learner?.students?.name||"Learner",pdf=new jsPDF({orientation:"portrait",unit:"mm",format:"a4",compress:true});pdf.setProperties({title:`${learner} Holistic Progress Card`,subject:"Final annual HPC"});pdf.setFontSize(18);pdf.text("EduAI Holistic Progress Card",18,20);pdf.setFontSize(10);pdf.text(`${learner} · Grade ${data.learner.grade} · ${data.learner.academic_year}`,18,29);pdf.text(`Framework: ${data.framework?.version_label||data.summary.framework_version_id}`,18,35);pdf.line(18,39,192,39);pdf.setFontSize(13);pdf.text("Teacher-approved annual narrative",18,49);pdf.setFontSize(10);pdf.text(pdf.splitTextToSize(data.summary.narrative_text||"",174),18,57);let y=92;pdf.setFontSize(12);pdf.text("Approved holistic results",18,y);y+=7;for(const a of data.assessments||[]){pdf.setFontSize(9);pdf.text(`${a.hpc_abilities?.label||"Ability"} · ${a.perspective}: ${a.teacher_override_level||a.calculated_level}`,20,y);y+=5}pdf.setFontSize(9);pdf.text(`Evidence snapshot: ${data.evidence?.length||data.summary.validation_json?.approvedEvidenceCount||0} approved item(s)`,18,270);pdf.text(`Approval: ${data.summary.finalized_at||"Draft"}`,18,276);const bytes=pdf.output("arraybuffer"),signature=String.fromCharCode(...new Uint8Array(bytes).slice(0,5));if(signature!=="%PDF-"){setMessage("PDF validation failed; no file was downloaded.");return}const url=URL.createObjectURL(new Blob([bytes],{type:"application/pdf"})),link=document.createElement("a");link.href=url;link.download=`${learner.replace(/[^a-z0-9]+/gi,"-")}-HPC-${data.learner.academic_year}.pdf`;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000)};
  const shareFinal=async()=>{const r=await authFetch("/api/hpc/shares",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({learnerId,type:"final_hpc",expiresInDays:14})}),p=await r.json();if(!r.ok){setMessage(p.error);return}await navigator.clipboard.writeText(p.url);setMessage("Secure final HPC parent link copied. It expires in 14 days.")};
  const checks=data?.validation?.checks||data?.summary?.validation_json?.checks||[];
  return <section className="card span-2 hpc-prompt-one hpc-annual"><CardHead eyebrow="Prompt 5 · annual finalization" title="Annual Holistic Progress Card and reporting"/><p className="modal-copy">Only approved HPC evidence is used. Academic X-Ray marks remain separate and no learner ranking or combined score is produced.</p><Field label="Learner"><select value={learnerId} onChange={e=>setLearnerId(e.target.value)}>{learners.map(l=><option key={l.id} value={l.id}>{l.students?.name||"Learner"} · Grade {l.grade}</option>)}</select></Field>{data&&<><div className="hpc-finalization-grid"><div><b>Finalization readiness</b>{checks.map((c:any)=><span key={c.code} className={c.passed?"check-pass":"check-fail"}>{c.passed?"✓":"!"} {c.label}<small>{c.detail}</small></span>)}</div><div><b>Version and evidence</b><span>{data.framework?.version_label||"Framework unavailable"}</span><span>{data.rule?.rule_code||"Official scoring not resolved"}</span><span>{data.evidence?.length||0} approved evidence item(s)</span><span>{data.summary?.status||"Not started"}</span></div></div><form key={`${learnerId}-${data.summary?.updated_at||"new"}`} className="hpc-subform" onSubmit={e=>{e.preventDefault();void submit(e.currentTarget,"draft")}}><b>Evidence-grounded narrative draft</b><label>Annual narrative<textarea name="narrative" minLength={20} defaultValue={data.summary?.narrative_text||""} placeholder="Generate a draft or write a teacher-authored synthesis using approved evidence only."/></label><div className="hpc-prompt-grid"><label>Strengths<textarea name="strengths" defaultValue={data.summary?.strengths_text||data.feedback?.strengths_text||""}/></label><label>Support next steps<textarea name="support" defaultValue={data.summary?.support_text||data.feedback?.support_text||""}/></label><label>Teacher notes<textarea name="notes" defaultValue={data.summary?.teacher_notes||""}/></label></div><label className="check"><input type="checkbox" name="approval" disabled={data.summary?.status==="finalized"}/> I confirm this report uses approved evidence and is ready for finalization.</label><div className="button-row"><button className="secondary" disabled={busy||data.summary?.status==="finalized"}>Save / generate draft</button><button className="secondary" type="button" disabled={busy||data.summary?.status==="finalized"} onClick={e=>void submit(e.currentTarget.closest("form") as HTMLFormElement,"validate")}>Run checks</button><button className="primary" type="button" disabled={busy||data.summary?.status==="finalized"} onClick={e=>void submit(e.currentTarget.closest("form") as HTMLFormElement,"finalize")}>Finalize immutable HPC</button>{data.summary?.status==="finalized"&&<><button className="secondary" type="button" onClick={download}>Download verified PDF</button><button className="secondary" type="button" onClick={()=>void shareFinal()}>Copy secure parent link</button></>}</div></form></>}{principal&&<div className="hpc-principal"><b>Principal aggregate dashboard · sample size {principal.sampleSize}</b><p>{principal.context}</p><div className="hpc-principal-grid">{principal.rows.map((row:any)=><article key={`${row.grade}-${row.section}-${row.academicYear}`}><strong>Grade {row.grade} · {row.section}</strong><span>{row.completionRate}% final HPC completion</span><span>{row.evidenceApproved} approved / {row.evidencePending} pending evidence</span><span>{row.insufficient} evidence sufficiency gap(s)</span><span>{row.participationRate}% perspective participation</span><span>{row.appliedComplete}/{row.appliedTotal} applied learning complete</span><span>{row.supportActive} active support action(s)</span></article>)}</div></div>}{busy&&<p className="modal-copy">Updating the annual HPC workspace…</p>}{message&&<p className={message.includes("finalized")||message.includes("saved")||message.includes("refreshed")?"success-note":"form-error"} role="status">{message}</p>}</section>;
}
...
function HpcPromptThreeWorkspace(){
  ...
  const load=()=>{const request=++requestId.current;setProgress(null);setFeedback(null);setMessage("");if(!learnerId)return;void Promise.all([authFetch(`/api/hpc/progress?learnerId=${encodeURIComponent(learnerId)}`),authFetch(`/api/hpc/feedback?learnerId=${encodeURIComponent(learnerId)}`)]).then(async responses=>{const payloads=await Promise.all(responses.map(response=>response.json()));if(request!==requestId.current)return;if(payloads[0].error)throw new Error(payloads[0].error);setProgress(payloads[0]);setFeedback(payloads[1].feedback||{})}).catch(error=>{if(request===requestId.current)setMessage(error instanceof Error?error.message:"Unable to load official progress.")})};
...
function Work({state,setState,selected,openAssessment,open,update,notify}:any){
  ...
  const list=state.assessments.filter((a:any)=>filter==="All"||stageLabel[a.stage as Stage].includes(filter));
  return <><PageHead eyebrow="Teacher workspace" title="Work & evidence" subtitle="Create the assessment and attach its required reference documents before adding student answer sheets."><button className="primary" onClick={()=>open("create-assessment")}>＋ Create assessment</button></PageHead>
    ...
    <div className="dashboard-grid"><section className="card span-2"><div className="filters">{["All","Draft","Uploaded","Review","Published"].map(x=><button key={x} className={filter===x?"active":""} onClick={()=>setFilter(x)}>{x}</button>)}</div><div className="work-grid">{list.map((a:any)=><button className={`work-card ${selected?.id===a.id?"selected-card":""}`} key={a.id} onClick={()=>openAssessment(a.id)}><div className="mini-paper"><i/><i/><i/></div><b>{a.title}</b><small>Class {a.grade}{a.section} · {a.subject}</small><Progress value={Math.round(stageProgress(a.stage))}/><span>{stageLabel[a.stage as Stage]} →</span></button>)}</div></section>
...
function ResourcesView({state,setState,open,notify}:any){
  ...
  const classes=Array.from(new Set<string>(state.assessments.map((a:Assessment)=>`Class ${a.grade}${a.section}`))).sort();
  const subjects=Array.from(new Set<string>(state.assessments.filter((a:Assessment)=>classFilter==="All classes"||`Class ${a.grade}${a.section}`===classFilter).map((a:Assessment)=>a.subject))).sort();
