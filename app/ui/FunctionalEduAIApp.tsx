...
function Reports({state,open,notify}:any){
  ...
  const assessmentsWithResults:Assessment[]=state.assessments.filter((assessment:Assessment)=>["approved","xray","intervention","followup","published"].includes(assessment.stage)&&Object.keys(assessment.gradeResults||{}).length>0);
  const [marksheetAssessmentId,setMarksheetAssessmentId]=useState("");
  const marksheetAssessment=assessmentsWithResults.find(assessment=>assessment.id===marksheetAssessmentId)||assessmentsWithResults[0];
  const concepts=conceptMastery(state);
  ...
  {tab==="Performance matrix report"?<><section className="card"><CardHead eyebrow="Performance metrics" title="Assessment marksheet"/>{marksheetAssessment?<div className="button-row"><label>Assessment<select value={marksheetAssessment.id} onChange={event=>setMarksheetAssessmentId(event.target.value)}>{assessmentsWithResults.map(assessment=><option key={assessment.id} value={assessment.id}>{assessment.title} · Class {assessment.grade}{assessment.section}</option>)}</select></label><button className="secondary" onClick={()=>void downloadAssessmentMarksheet(marksheetAssessment)}>Download Marksheet</button></div>:<p className="modal-copy">Final teacher-reviewed marks will appear here after an assessment is reviewed.</p>}</section><SchoolPerformanceMatrix state={state}/></>:<div className="dashboard-grid"><section className="card span-2"><CardHead eyebrow={tab} title={reportTitle(tab)}><button className="secondary" onClick={()=>notify(`${tab} exported as a report`)}>Export</button></CardHead>
...
function resourceVisual(topic:string,visual?:{type?:string;title?:string;items?:string[]}){if(!visual?.items?.length)return "";const items=visual.items.slice(0,5).map(item=>`<li>${htmlEscape(item)}</li>`).join("");return visual.type==="flowchart"?`<h3>${htmlEscape(visual.title||`${topic} flowchart`)}</h3><table class="visual-flow"><tr>${visual.items.slice(0,3).map((item,index)=>`<td><b>${htmlEscape(item)}</b></td>${index<Math.min(2,visual.items.length-1)?'<td class="arrow">→</td>':''}`).join("")}</tr></table>`:`<h3>${htmlEscape(visual.title||`${topic} mind map`)}</h3><div class="answer"><b>${htmlEscape(topic)}</b><ul>${items}</ul></div>`}
function answerKeyDocumentBody(content?:{mcqQuestions:{question:string;options:string[];correctIndex:number;concept?:string}[];subjectiveQuestions:{question:string;modelAnswer:string;concept?:string}[]}){
  ...
  const concepts=Array.from(new Set([...(content?.mcqQuestions||[]),...(content?.subjectiveQuestions||[])].map((question:any)=>question.concept).filter(Boolean)));
  return `<h2>Multiple-choice answers</h2>${mcqKey||"<p>None</p>"}<h2>Written-response marking guide</h2>${subjKey||"<p>None</p>"}${concepts.length?resourceVisual("Key concepts",{type:"mind_map",title:"Answer key concept map",items:concepts}):""}`;
}
...
async function downloadAssessmentMarksheet(assessment:Assessment){
  const XLSX=await import("xlsx");
  const rows=Object.values(assessment.gradeResults||{}).map((result:GradeResult)=>({"Student Name":result.studentName,"Marks Obtained":result.score,"Total Marks":result.maxMarks})).sort((a,b)=>a["Student Name"].localeCompare(b["Student Name"]));
  const sheet=XLSX.utils.aoa_to_sheet([["Assessment Name",assessment.title],["Class",`Class ${assessment.grade}`],["Division",assessment.section],[],["Student Name","Marks Obtained","Total Marks"]]);
  XLSX.utils.sheet_add_json(sheet,rows,{origin:"A6",skipHeader:true});
  sheet["!cols"]=[{wch:30},{wch:18},{wch:15}];
  const workbook=XLSX.utils.book_new();XLSX.utils.book_append_sheet(workbook,sheet,"Marksheet");
  XLSX.writeFile(workbook,`${safeDownloadName(assessment.title)}-Marksheet.xlsx`);
}
