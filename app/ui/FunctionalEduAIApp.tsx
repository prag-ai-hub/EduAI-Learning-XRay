...
function HpcAppliedLearning(){
  ...
  const saveStage=async(form:HTMLFormElement)=>{const f=new FormData(form),r=await authFetch(`/api/hpc/applied-learning/${encodeURIComponent(stageRecordId)}/stages`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({stageNumber:f.get("stageNumber"),status:f.get("stageStatus"),dueDate:f.get("dueDate"),learnerReflection:f.get("stageReflection"),teacherAssessment:f.get("stageAssessment"),peerFeedback:f.get("stagePeer")})}),p=await r.json();setMessage(r.ok?`Stage ${p.stage.stage_number} updated.`:p.error);if(r.ok)load()};
...
function HpcAppliedLearningDetails(){
  ...
  useEffect(()=>{void authFetch("/api/hpc/learners").then(r=>r.json()).then(p=>{const list=p.learners||[];setLearners(list);setLearnerId(list.find((x:any)=>Number(x.grade)>=9)?.id||"")})},[]);useEffect(loadRecords,[learnerId]);useEffect(loadDetail,[recordId]);
