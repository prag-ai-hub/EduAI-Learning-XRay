...
function Review({selected,update,notify,open}:any){
  ...
  const submitReview=()=>{if(!current||pendingCount)return;saveDrafts(drafts);update(assessment.id,{stage:"approved",reviewed:questions.length,totalReviews:questions.length});notify("Review submitted. Teacher-approved marks are now final for reports.")};
  ...
  if(!results.length)return <><PageHead eyebrow={assessment.title} title="Teacher grading review" subtitle="Compare original evidence, AI suggestions and the rubric before approval."><button className="secondary" onClick={()=>open("grade-picker")}>Grade an answer sheet</button></PageHead><section className="card span-2"><p className="eyebrow">No graded evidence yet</p><h2>Grade an answer sheet to review it here</h2><p>The continuous review workspace becomes available after an ungraded answer sheet completes AI grading.</p></section></>;
  return <><PageHead eyebrow={`${assessment.title} · Review workspace`} title="Teacher grading review" subtitle="Scroll through every page and question, compare the original evidence, and approve teacher marks."><button className="secondary" onClick={()=>open("bulk-review")}>← Back to Submissions</button><button className="secondary" onClick={()=>downloadStudentLearningGapReport(assessment,current,answerFile)}>Download</button><button className="primary" disabled={pendingCount>0} onClick={submitReview}>Submit Review</button></PageHead>
    <section className="review-student-bar card" aria-label="Student review navigation"><button className="secondary" disabled={studentIndex===0} onClick={()=>setStudentIndex(value=>Math.max(0,value-1))}>← Previous Student</button><div><select aria-label="Current student" value={studentIndex} onChange={event=>setStudentIndex(Number(event.target.value))}>{results.map((result,index)=><option key={result.fileId} value={index}>{result.studentName}</option>)}</select><b>Class {assessment.grade}{assessment.section}</b><small>{assessment.subject} · {assessment.title}</small></div><button className="secondary" disabled={studentIndex===results.length-1} onClick={()=>setStudentIndex(value=>Math.min(results.length-1,value+1))}>Next Student →</button></section>
    <section className="review-summary" aria-label="Overall Summary"><article><span>AI Total Marks</span><b>{aiTotal} / {current.maxMarks}</b></article><article><span>Teacher Reviewed Marks</span><b>{teacherTotal} / {current.maxMarks}</b></article><article><span>Pending Review</span><b>{pendingCount}</b></article><article><span>Overall progress</span><b>{questions.length?Math.round(reviewedCount/questions.length*100):0}%</b><Progress value={questions.length?Math.round(reviewedCount/questions.length*100):0}/></article></section>
