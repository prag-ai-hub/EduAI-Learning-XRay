import { getAuthorizedProfile } from "@/server/authorization";
import { validateEvaluationSubmission, sha256, type EvaluationSubmission } from "@/server/evaluator-grading";
  ...
  try {
    ...
    if (foundationMissing) {
      ...
      const audit = await db.from("audit_events").insert({
        ...
        // No `snapshot`. It carried the whole canonical evaluation - the
        // student's name, per-question evidence and AI rationale - into a table
        // whose contract (backend/apps/platform/audit/services.py) is that it
        // holds no sensitive payload, and which SchoolAdmins can read. Nothing
        // ever read it back: replay below needs `evaluation` only, which is ids,
        // totals, a content hash and a status. The hash is what ties this record
        // to the submitted content, and it is not reversible.
        detail_json: { idempotencyKey: submission.idempotencyKey, assessmentId: submission.assessmentId, fileId: submission.fileId, evaluation, compatibilityStore: true },
      });
