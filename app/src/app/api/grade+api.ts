...
import { aiProxyConfigured, complete } from "@/server/ai-proxy";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const {getSupabaseServer}=await import("@/server/supabase-server");
...
Produce the CBSE diagnostic result and exclude fully correct questions from gaps. When a reanalysis reason is supplied, explicitly reconsider that feedback while remaining grounded in the validated evidence.`;
    ...
    if(chargedOperation&&chargedUserId){try{const {getSupabaseServer}=await import("@/server/supabase-server");await getSupabaseServer().rpc("refund_credit",{p_user_id:chargedUserId,p_operation_key:chargedOperation,p_reason:error instanceof Error?error.message:"Analysis failed"})}catch{}}
