import { ocr as ocrViaProxy } from "@/server/ai-proxy";
...
/**
 * Spreadsheet text via SheetJS, pinned in package.json to the vendor's 0.20.3
 * tarball. The npm registry is frozen at 0.18.5, which carries two unfixable
 * high-severity advisories against exactly this code path; the reasoning for a
 * non-registry URL is in docs/DEPENDENCIES.md.
 *
 * Sheet names come out of the uploaded file, so they are attacker-controlled
 * keys. `Sheets["__proto__"]` still resolves through the prototype rather than
 * to a sheet on 0.20.3, so a name is honoured only when it is genuinely an own
 * key: dropping a sheet nobody legitimately names beats handing the prototype
 * chain to sheet_to_csv.
 */
async function extractSpreadsheet(bytes: Uint8Array) {
  const XLSX = await import("xlsx");
  const workbook = XLSX.read(bytes, { type: "array" });
  const isOwnSheet = (name: string) => Object.prototype.hasOwnProperty.call(workbook.Sheets, name);
  return workbook.SheetNames.filter(isOwnSheet).map(name => `# ${name}\n${XLSX.utils.sheet_to_csv(workbook.Sheets[name])}`).join("\n\n").trim();
}
