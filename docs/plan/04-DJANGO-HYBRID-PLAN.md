...
| 13.3 | Parent dashboard: reports/interventions API | 2.5 | ✔ **Done 2026-09-15.** The API had already landed unmarked: `GET /parents/reports` (every linked child) and `GET /parents/children/{id}/reports/`, both reading `public.parent_child_reports` (M11) - the field whitelist, so `ocr_text`, `question_decisions_json` and AI rationale are absent rather than filtered. Interventions ride along as **`classInterventions`**: `public.interventions` has an `assessment_id` and no `student_id`, so two siblings in one class get the same list, and the key and the UI heading both say it is the class's plan. What was missing was the client: `/parent` still read an Expo server route calling the same function with the **service-role key** - right answer, no capability check, throttle or audit. It now reads Django, renders the interventions it never showed, and that route is **deleted**, not kept as a fallback. Fixed on the way: the Django client took its token only from the build-time Supabase singleton, so a build without `EXPO_PUBLIC_SUPABASE_*` had every Django call refused while `authFetch` worked; it now shares `authFetch`'s token. Verified over HTTP: parent 200 with interventions and no canaries, teacher 403 |
| 14.1–14.2 | Frontend: B2B checkout UI, parent dashboard UI | 6.0 | ✔ **Done 2026-09-15.** **14.1:** a Billing module in the SchoolAdmin workspace - current plan and entitlement, the catalogue, GST invoice details (state picker for `place_of_supply`, GSTIN checked against it), the server-priced total with GST shown **before** the payment sheet opens, then Razorpay. It never shows success on the browser's callback: it polls the payment until the signed webhook has captured it, and says so if that is slow. One idempotency key per attempt. Stop-renewal behind a confirmation. The payment sheet is web-only; a device says so rather than half-working - native checkout needs `react-native-razorpay` and a development build, which is a product decision. **14.2:** the dashboard from 12.2/13.3 gained "Remove from my account" (`POST children/{id}/unlink/`), confirmed, with copy that says an old code will not restore access (M18). Both driven in Chrome against the local stack: sign-in, billing through to the gateway's refusal (no keys locally), no page errors |
...
| 15.3 | Plans & pricing schema finalization | 2.5 | ✔ **Done 2026-09-18.** The owner supplied the pricing table, and it is not a subscription product: X-Ray Free/Single/Multi/Bulk are **capacity packs** sold in DIY and DFY variants, so the catalogue is now seven real plans and `PRICING_CONFIRMED` is True. Owner's answers, implemented: capacity lasts **12 months** then lapses (`PACK_VALIDITY_MONTHS`; the subscription row carries `cancel_at_period_end`, which is how that table says "does not renew"); a **credit is one student's answer sheet**, so a pack's `credits_included` is its coverage (50/250/500); **X-Ray Free is granted on approval** with no checkout, and `start_school_checkout` refuses any zero-amount plan because a ₹0 order at the gateway is an error. **M20** relaxes `plans_amount_paise_check` to allow the free tier to exist as a row. Two real gaps closed on the way: a captured school payment granted **no credits at all** (the number on the pricing page bought nothing), and the old placeholder plans would have stayed on sale because the seeder only touches codes in its own file - they are archived explicitly. Credits land on the school's first administrator, who holds "Assign credits"; `payments` has no payer column, so that is a decision to confirm rather than derive. 11 new tests; verified over HTTP |

### Week 4 — Payment completion, testing, deployment (42.5 hrs)

| Day | Task | Hrs | Status |
| --- | --- | ---: | --- |
| 16.1 | Webhook handler: success/failure + signature verification | 4.0 | ✔ **Built 2026-09-09, marked 2026-09-15.** `POST /billing/webhooks/razorpay`: HMAC-SHA256 over the **raw** bytes, refused before any row is read - missing, wrong-secret, re-serialised-body and hostile signature headers all move nothing (and a hostile header is a 4xx, not a 500). A payload charging a different amount never captures; a failure after a capture does not move it back. `test_webhooks.py`, 42 tests |
| 16.2 | Webhook: renewal/cancellation + idempotency | 3.0 | ✔ **Built 2026-09-09, marked 2026-09-15.** Renewing the same plan extends rather than restarts; renewing an expired period starts from now. The same capture applied twice, or redelivered under a new event id, grants nothing twice; a transient failure answers 5xx so the gateway's redelivery replays it. Cancellation keeps the period already paid for. On top of M10 `payment_events` and M15 |
| 16.3 | Rate-limit testing & threshold tuning | 1.5 | ✔ **Done 2026-09-17.** Enforcement was already covered by four tests in `apps/common/tests/test_throttling.py`; what was missing was **coverage**. `ScopedRateThrottle` is the only default class and is a **no-op on a view with no `throttle_scope`**, so a view shipped unthrottled broke nothing and no test noticed - the suite runs with throttling off by design. `tests/test_rate_limits.py` (15) now walks the real URLconf and fails on any route with no effective limit (`/health` and DRF's router index are exempt, with reasons; proved by removing a scope and watching it name the view), and checks every declared scope has a rate - a typo is a 500 on the first request, not a loose limit. Four enforcement tests joined the existing file rather than starting a second helper: ten sign-ups then 429, the standard envelope with `Retry-After`, one shared allowance per scope, and the refusal landing **before** the handler works. Tuning: thresholds moved to `apps/common/throttling.py` with the reasoning beside each number and the settings importing it, so the tests and the live config cannot drift; the dead `anon: 30/min` was removed (nothing used `AnonRateThrottle`, so it implied protection that did not exist) and `parent_link` is now configured there rather than surviving only as a hardcoded fallback |
| 17.1 | Payment receipts + history APIs | 3.0 | ✔ **Built 2026-09-09, marked 2026-09-15.** `GET /billing/payments/`, `/receipts/`, `/invoices/`. A school admin sees only their school's, a parent only their own, a Teacher nothing; a SuperAdmin must name the school and needs a live support grant, which is audited. `test_receipts.py` 33, `test_billing_reads.py` 23 |
| 17.2 | GST-ready PDF invoice generation + compliance fields | 3.5 | ✔ **Built 2026-09-09, marked 2026-09-15.** `GET /billing/invoices/{id}/pdf/`, ReportLab. Gapless numbering per financial year under a row lock, within GST's 16-character limit; a rolled-back issue takes its number with it; issuing twice returns the first invoice. Intra-state CGST+SGST, inter-state IGST, both adding back to the same total. Every string is escaped into the PDF (markup injection was a real finding). `test_invoices.py` 42, `test_gst.py` 8 |
| 17.3 | Failed-payment retry / grace period | 2.0 | ✔ **Built 2026-09-09, marked 2026-09-15.** A failed renewal opens a grace window instead of cutting the school off; a second failure does not extend it; a successful retry closes it; `past_due` entitles only inside the window, and the boundary is tested on both sides. The retry itself is the gateway's; this is what the product does while it happens |
| 17.4 | **School roster: classes & students API + UI** (added 2026-09-17) | 6.0 | ✔ **Backend done 2026-09-17.** Found while using the product: "Add class", "Edit school" and the roster import wrote to the **workspace JSON blob** and made no network call at all, so a school could not create a class, a student, or anything the rest of the product could see - and `classes`/`students` had models but no endpoints. Nothing in this plan covered it. `/schools/classes/` and `/schools/students/` (+ `students/import/`), tenant-scoped, `school.classes.manage` / `school.roster.manage`, writes gated on an Active school (matrix §5). The security case is that the request carries only an **id**: a teacher or class from another school is refused with the same answer as one that does not exist. **M19** adds the identity the tables never had - unique (school, year, class, section, subject), a per-school partial unique roll number, and a two-value status check - because a serializer cannot close that race. Removing a student marks them Inactive: grade results, parent links and audit rows point at the id. Import resolves spreadsheet labels ("6C", "6-c"), updates on a matching roll number rather than duplicating, refuses an ambiguous label rather than guessing a subject, and is one transaction. 36 tests; verified over HTTP. **UI done the same day:** the four screens now read and write the API (`features/roster/api/rosterApi.ts`), with the workspace blob kept as a cache for the screens not yet ported. `PATCH /schools/mine` closes the third half of the same complaint - the matrix grants `school.profile.edit` and there was no endpoint; `status` is not a field on it, because a school that could PATCH its own status could approve itself. Driving it in Chrome found **a bug that made every dialog in the app unusable on web**: `ModalShell` nested the panel inside the backdrop `Pressable` and relied on the touch responder to stop presses, which holds on native and not on react-native-web - so clicking any field closed the dialog. The backdrop is now a sibling, so there is no ancestor to bubble to. Also fixed `toApiError`, which flattened `{rows:[{row,detail}]}` with `String(value)` and turned every structured error into "[object Object]" |
| 18.1 | Frontend: receipts/history UI + top-up wiring | 3.0 | ▢ |
| 18.2 | Automated tests: RBAC, JWT, tenant isolation, webhook idempotency | 4.0 | ✔ **2026-09-15.** 953 backend + 182 frontend contract tests green. RBAC: `test_endpoint_authorisation.py` (192) walks the real URLconf so a new ungated view fails the build. JWT: `test_authentication.py` (17) - expiry, audience, wrong secret, `alg: none`, an ES256 token from the wrong key, and header algorithms outside the allowlist. Tenant isolation: `test_tenancy.py` (27) plus per-surface scoping suites. Webhook idempotency: `test_webhooks.py` (42) |
